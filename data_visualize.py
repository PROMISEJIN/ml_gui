...
import numpy as np
from sklearn.preprocessing import LabelEncoder, StandardScaler, MinMaxScaler, PowerTransformer # PowerTransformer 파워 스케일링
...
class data_:  
    ...
    def scatter_plot(self,df,x,y,c,marker):
        plt.figure()
        plt.scatter(df[x],df[y],marker=marker,c=c)
        plt.xlabel(x)
        plt.ylabel(y)
        plt.title(y + " vs "+ x)
        plt.show()
    ...
    def line_plot(self,df,x,y,c,marker):
        plt.figure()
        plt.plot(df[x],df[y],marker=marker,c=c)
        plt.xlabel(x)
        plt.ylabel(y)
        plt.title(y + " vs "+ x)
        plt.show()
